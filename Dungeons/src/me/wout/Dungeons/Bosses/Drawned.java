...
public class Drawned implements Listener {

	private main plugin;
	private Set<UUID> drawneds = new HashSet<UUID>();
	private Set<UUID> guardians = new HashSet<UUID>();
	Location spawnloc;
	BossBar bossbar;
	
	public Drawned(main plugin) {
		this.plugin = plugin;
	}
		...
		if (event.getHitEntity() instanceof Player && event.getEntity() instanceof Trident) {
			if (event.getEntity().getShooter() instanceof Drowned) {
				...
				if (drawneds.contains(drawned.getUniqueId())) {
					...
					event.getHitEntity().setLastDamageCause(new EntityDamageEvent(event.getHitEntity(), DamageCause.ENTITY_ATTACK, 4.0));
