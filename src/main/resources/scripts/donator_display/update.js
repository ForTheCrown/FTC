...
color COLOR = NamedTextColor.NAMES.valueOrThrow(args[1]);
...
function pushStandsBack() {
  for (let i = armorStands.length-2; i >= 0; i--) {
    ...
    if (ItemStacks.isEmpty(nEquip.getHelmet())) {
      sEquip.setHelmet(null);
      sEquip.setLeggings(null);
      sEquip.setBoots(null);
    }
...
function giveArmor(stand) {
  ...
  eq.setChestPlace(new ItemStack(Material.GOLD_CHESTPLATE));
