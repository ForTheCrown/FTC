...
public abstract class HelpCommand extends FtcCommand {
    ...
    public void createCommand(GrenadierCommand command) {
      command.executes(c -> {
        ...
        if (loc.getWorld().equals(Worlds.resource())) {
          ...
          player.sendMessage(
              ChatColor.GRAY + "Try " + ChatColor.YELLOW + "/warp portal" + ChatColor.GRAY
                  + " to get back to the normal world.");
          player.sendMessage(
              ChatColor.GRAY + "Type " + ChatColor.YELLOW + "/posthelp" + ChatColor.GRAY
                  + " for more help.");
          return 0;
  ...
  public static class HelpShop extends HelpCommand {
    ...
    @Override
    public void writeDisplay(TextWriter writer, User user) {
      Component editMessage = Component.text("[editshop]")
          .clickEvent(ClickEvent.runCommand("/editshop"));
      ...
      Component message = Component.text()
          ...
          .append((user.hasPermission(Permissions.SHOP_EDIT) ? editMessage
              : Component.text("/editshop", NamedTextColor.YELLOW)))
