...
/**
 * A component of a boss, allows for easily attaching features to a boss
 *
...
public interface BossComponent<T extends DungeonBoss> extends Listener {
  ...
  /**
   * Ran right after a boss is killed, at this point the boss entity is still accessible
   *
   * @param boss    The boss that died
   * @param context The fight's context
   * @param forced  True, if the death was forced, like by a server restart or something similar
   */
  default void onDeath(T boss, BossContext context, boolean forced) {
  }

  /**
   * Called whenever the boss is hit, depending on boss type, the entity damage event may be null
   *
   * @param boss    The boss that was hit
   * @param context The fight's context
   * @param event   The damage event in which the boss was damaged
   */
