 ...
 * <p>
 * This uses a system of LoadSections, which takes the area the loader has to load and splits it
 * between square sections called a LoadSection, its size is determined by SECTION_SIZE. All
 * sections run their loading at the same time in parallel. The created semaphore ensures that these
 * sections don't overwhelm the CPU and RAM with chunk load requests
 */
public @UtilityClass class WorldLoader {
  ...
  /**
   * The maximum world size that can be loaded, anything above this takes too long and most likely
   * generates a world file too large.
   */
  public final double MAX_LOADABLE_AREA = 50_000D;

  /**
   * The semaphore that ensures we never load more than {@link #MAX_LOADING_CHUNKS} at a time.
   * <p>
   * If we passed this limit, there's potential for the server's main thread to die or freeze to a
   * point where it can't continue, so we need to limit ourselves.
   */
  ...
  /**
   * The loader's world load tracker. It maps all loaded worlds by their world key to the instance
   * of the loader currently running on that world.
   * <p>
   * Tracking required if we'd need to shut down or otherwise interact with any in-progress loader
   * instance.
   */
   ...
   * @return A completable future that will be complete once the entire world has finished loading
   */
  ...
  /**
   * Load progress class used for tracking and logging the progress of a world load.
   */
  private class LoadProgress {
    ...
    /**
     * The amount of chunks that the last log had
     */
  ...
  /**
   * A single section of a world that's being loaded by the {@link LoaderInstance}
   */
  private class LoadSection implements Runnable {
    ...
    public String toString() {
      return getClass().getSimpleName() + "{start=" + start + ", end=" + new ChunkPos(
          start.x + SECTION_SIZE, start.z + SECTION_SIZE) + "}";
    }
  }
}