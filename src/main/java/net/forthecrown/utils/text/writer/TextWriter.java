 ...
 * <p>
 * Currently, there are text writers like {@link LoreWriter} which depend on {@link #onNewLine()}
...
public abstract class TextWriter implements ComponentLike {
  ...
  /**
   * Writes the given string as a single line with an empty style.
   * <p>
   * If {@link #isLineEmpty()} returns false, then this will call {@link #newLine()} and then write
   * the given text. After the given text is written, {@link #newLine()} is not called again by this
   * method.
   *
  ...
  /**
   * Writes the given string as a single line with the given text
   * <p>
   * If {@link #isLineEmpty()} returns false, then this will call {@link #newLine()} and then write
   * the given text. After the given text is written, {@link #newLine()} is not called again by this
   * method.
  ...
  /**
   * Writes the given string with the given style as a single line.
   * <p>
   * If {@link #isLineEmpty()} returns false, then this will call {@link #newLine()} and then write
   * the given text. After the given text is written, {@link #newLine()} is not called again by this
   * method.
   ...
   * the writer's results. One example of this is {@link LoreWriter}
   */
  public void newLine() {
    onNewLine();
    lineEmpty = true;
   ...
   *
   * @param s     The string to write
   * @param style The style to use for the text being written, this will be applied after the string
   *              is converted to a Component
   */
  ...
  /**
   * Writes the given string with the given color as the base color
   *
   * @param s     The string to write
  ...
  /**
   * Writes the given string to this writer with an empty style
   *
   * @param s The string to write
   */
  ...
  /**
   * Delegate method for {@link Text#format(String, Object...)}. Writes a formatted message to this
   * writer
   *
  ...
  /**
   * Delegate method for {@link Text#format(String, TextColor, Object...)}. Writes a formatted
  ...
  /**
   * Creates a writer which writes to the same source as this writer with a prefix after every new
   * line write.
   *
