 ...
 * <p>
 * {@link Header} for formatting a list's header, {@link PageEntry} for formatting each individual
 * entry on a page, {@link Footer} for formatting a page's footer, aka the page number and buttons
 * on the bottom.
 * <p>
 * In a way, this class just acts as a delegate for it's 3 components and ensures that the header
 * and each entry in the page, and footer are on different lines
 *
...
public class PageFormat<T> {
  ...
  /**
   * Formats the given page iterator into a displayable page
   *
  ...
  /**
   * Writes the given iterator into the given text writer.
   * <p>
   * If the given iterator has no entries, then will just write the header and footer right after
   * each other. If both the iterator have no entries and the header and footer are null, this
   * writes nothing
   *
  ...
  /**
   * Sets the header of this format to have the given title, separated by
   * {@link Messages#PAGE_BORDER} borders
   *
  ...
  /**
   * Sets the command format to use for the footer's page button provider
   * <p>
   * Please see {@link Footer#setPageButton(String)} for details about the format itself.
   *
   ...
   * Sets the entry formatter to use.
   * <p>
   * Creates an entry formatter using {@link PageEntry#of(PageEntry.EntryDisplay)} with the default
   * index formatter
   *
