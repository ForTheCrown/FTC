...
public final class PathUtil {
  ...
  /**
   * Macro for {@link #pluginPath(String, String...)} and {@link #ensureDirectoryExists(Path)} to
   * ensure a directory within the plugin's data folder exists.
   * <p>
   * Calls {@link #ensureDirectoryExists(Path)} with the given path name(s) and then calls
   * {@link Either#orThrow()} on the result, this will either return the input path or throw an
   * exception
   *
   * @param first  The first part of the path
   * @param others The rest
   * @return The created plugin
   * @throws RuntimeException If the path couldn't be created, or if the path was not a folder and
   *                          couldn't be deleted.
   */
