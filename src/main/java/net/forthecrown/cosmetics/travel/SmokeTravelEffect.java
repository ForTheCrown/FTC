...
public class SmokeTravelEffect extends TravelEffect {
  ...
  public void onHulkStart(User user, Location loc) {
    ...
    loc.getWorld()
        .spawnParticle(Particle.EXPLOSION_LARGE, loc.add(0, 0.1, 0), 1, 0, 0, 0, 0, null, true);
  }
  ...
  public void onHulkTickDown(User user, Location loc) {
    ...
    loc.getWorld()
        .spawnParticle(Particle.CAMPFIRE_COSY_SMOKE, loc.add(0, 3, 0), 2, 0, 0.5, 0, 0.01, null,
            true);
  }
  ...
  public void onHulkTickUp(User user, Location loc) {
    // Smokes following user
    loc.getWorld()
        .spawnParticle(Particle.CAMPFIRE_COSY_SMOKE, loc.add(0, -1, 0), 3, 0, 0.5, 0, 0.01, null,
            true);
  }
  ...
  public void onHulkLand(User user, Location landing) {
    // Little ball of smoke particles
    landing.getWorld()
        .spawnParticle(Particle.CAMPFIRE_COSY_SMOKE, landing.add(0, 0.1, 0), 20, 0.2D, 0, 0.2D,
            0.01D, null, true);
  }

  void ninjaBomb(Location loc) {
    loc.getWorld()
        .spawnParticle(Particle.CAMPFIRE_COSY_SMOKE, loc.add(0, 0.1, 0), 100, 0.5D, 1, 0.5D, 0.01D,
            null, true);
  }
}