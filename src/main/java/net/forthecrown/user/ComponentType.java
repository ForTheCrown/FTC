 ...
 *
 * @param <T> The user component implementation class this type represents
 * @see Components
 */
@Getter
public class ComponentType<T extends UserComponent> {
  ...
  /**
   * The parameters a {@link UserComponent} implementation needs to be automatically constructed
   */
  ...
  /**
   * The type's runtime array index used to speed up lookups in the {@link User} class
   */
  ...
  /**
   * Gets the constructor used to instantiate user component objects.
   *
   ...
   */
  private static <T extends UserComponent> Constructor<T> getConstructor(Class<T> typeClass)
    ...
    try {
      ...
      throw new IllegalArgumentException(
          String.format(
              "'%s' is missing a (%s, %s) constructor required for automatic class instantiation",
   ...
   * @throws IllegalArgumentException If this component type's component class does not have a valid
   *                                  constructor
   * @throws IllegalStateException    If there was error with calling the component class'
   *                                  constructor
   */
