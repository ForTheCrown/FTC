...
/**
 * A class which stores the {@link UserProperty} instances as constants.
 */
public class Properties {
  ...
  /**
   * Determines whether the given user sees automated announcements or not.
   */
  IGNORING_ANNOUNCEMENTS = new BoolProperty("ignoringBroadcasts", false),

  /**
   * Determines whether a user will instantly teleport between poles or fly into the sky and then
   * drop down lol.
   */
  HULK_SMASHING = new BoolProperty("hulkSmashing", true),

  /**
   * Determines whether others can see a users profile.
   */
  PROFILE_PRIVATE = new BoolProperty("profilePrivate", false),

  /**
   * Determines if a player can TPA to other players or have other players TPA to them.
   */
  TPA = new BoolProperty("tpa", true),

  /**
   * Determines if a player can pay other players and be paid by other players.
   */
  PAY = new BoolProperty("paying", true),

  /**
   * Determines whether a user can send and receive region invites.
   */
   ...
   * <p>
   * If true, all of the player's messages will go to their spouse only, not the general chat.
   */
  MARRIAGE_CHAT = new BoolProperty("marriageChatToggle", false),

  /**
   * Determines if a user can send and receive marriage proposals.
   */
  ACCEPTING_PROPOSALS = new BoolProperty("acceptingProposals", false),

  /**
   * Determines if a user sees a muted user's chat messages with
  ...
  /**
   * Determines if a user sees what people write on signs with
   * {@link net.forthecrown.core.admin.EavesDropper}
   * <p>
   * Only affects users with {@link net.forthecrown.core.Permissions#EAVESDROP}
   */
  ...
  /**
   * Determines if a user sees what other married users send to each other through
   * {@link net.forthecrown.user.MarriageMessage} with
   * {@link net.forthecrown.core.admin.EavesDropper}
   * <p>
   * Only affects users with {@link net.forthecrown.core.Permissions#EAVESDROP}
   */
  ...
  /**
   * Determines if a user sees eaves dropper messages when other users mine into veins. Used by
   * {@link net.forthecrown.core.admin.EavesDropper}
   * <p>
  ...
  /**
   * /shop property, determines if the selling system should also sell named items, or ignore them.
   */
  SELLING_NAMED_ITEMS = new BoolProperty("sellingNamedItems", false),

  /**
   * /shop property, determines if the selling system should also sell items with lore or ignore
   * them.
   */
  SELLING_LORE_ITEMS = new BoolProperty("sellingLoreItems", false),

  /**
   * /shop property, determines whether the user is selling items in their compact form or not
   */
  SELLING_COMPACTED = new BoolProperty("sellingCompact", false),

  /**
   * Determines if a user should be shown other users' staff notes when they join.
   * <p>
   * This property only affects uses that have the staff permissions, it will not affect regular
   * users.
   */
  VIEWS_NOTES = new BoolProperty("viewsNotes", true),

  /**
   * Determines if a user receives alerts when their gear or items pass a certain durability
   * threshold.
   *
  ...
  /**
   * Determines whether a user will see ranks besides name tags in chat and in the player name
   */
  ...
  /**
   * Determines the display name shown in the TAB menu, will be {@link Component#empty()} if not
   * set. If not set, the user's TAB display name will simply be their username or nickname
   */
