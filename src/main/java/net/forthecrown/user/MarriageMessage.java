...
import static net.forthecrown.core.Messages.MARRIAGE_POINTER;
import static net.forthecrown.core.Messages.MARRIAGE_PREFIX;
import static net.forthecrown.core.Messages.MC_BLOCKED_SENDER;
import static net.forthecrown.core.Messages.MC_BLOCKED_TARGET;
...
public class MarriageMessage {
  ...
  /**
   * The formatted message, if the sender doesn't have permissions for color-coded chat messages or
   * emojis, this will basically just be {@link #input} as a plain text component
   */
  private final Component formattedText;

  /**
   * True, if this marriage message was sent in normal chat by a user with
   * {@link Properties#MARRIAGE_CHAT} enabled. If false, this was sent via command.
   */
  ...
  /**
   * Sends a marriage message from the given sender to the given target
   *
   ...
   * <p>
   * This will make sure the sender didn't use any banned words, isn't muted/softmuted and isn't
   * seperated or blocked by their spouse. If all those checks are passed, the target and sender are
   * both send the same formatted message
   */
  public void run() {
