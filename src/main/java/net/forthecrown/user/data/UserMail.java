...
public class UserMail extends UserComponent {
  ...
  /**
   * The list of messages that have been sent to the holder of this mail.
   * <p>
   * New entries will be inserted to the beginning of this list. Old entries, determined with
   * {@link #shouldRetainMessage(MailMessage)}, are not serialized or deserialized, but are also not
   * cleared, because I am lazy. - Jules
   */
  @Getter
  ...
  /**
   * Clears all mail messages that either don't have an attachment, or have a claimed attachment.
   *
   * @see MailAttachment
   */
  public void clearPartial() {
    mail.removeIf(
        message -> MailAttachment.isEmpty(message.getAttachment()) || message.getAttachment()
            .isClaimed());
  }
  ...
  /**
   * Adds a message with the given text and sender to this user's mail
   *
   * @param message The message to add
   ...
   * <p>
   * If the message has an attachment that's not claimed then this returns true, otherwise, it
   * tests, if the message has expired, in the sense that it was sent more than
   * {@link GeneralConfig#dataRetentionTime} time ago.
   *
   * @param message The message to test
   * @return True, if the message should be retained, false if it should be deleted
   */
