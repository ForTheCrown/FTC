 ...
 * <p>
 * The logger file is split between 2 major parts: The header, and the content. The header details
 * where in the file a log section is located and what sections are in a file.
 * <p>
 * The header begins with an integer size, stating how many sections the file contains. More
 * specifically, this data is stored in UTF-8, <code>long</code> pairs. The long value is an offset
 * of the section's beginning byte address from the end of the header
 * <p>
 * Sections are lists of {@link BinaryJson} data that begin with a size integer stating how many
 * entries there are. Individual entries are then deserialized and translated into entries by
 * {@link LogSchema} instances.
 */
public class LogFile {
  ...
  /**
   * Versions of each written section, Contains {@link #count} number of entries
   */
