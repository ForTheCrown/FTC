...
import static net.forthecrown.core.config.ResourceWorldConfig.legalSeeds;
import static net.forthecrown.core.config.ResourceWorldConfig.nextSize;
import static net.forthecrown.core.config.ResourceWorldConfig.resetEnd;
import static net.forthecrown.core.config.ResourceWorldConfig.resetInterval;
import static net.forthecrown.core.config.ResourceWorldConfig.resetStart;
import static net.forthecrown.core.config.ResourceWorldConfig.spawnStructure;
...
public class ResourceWorld {
  ...
  private void onWorldLoadedSync(World world, BlockStructure spawn) {
    ...
    if (toHaz == null) {
      LOGGER.warn("Could not re-position rw -> haz gate, could not find gate with id {}",
          toHazGate);
    } else {
    ...
    if (toRes == null) {
      LOGGER.warn("Could not re-position haz -> rw gate, could not find gate with id {}",
          toResGate);
    } else {
