...
/**
 * A single entry in a shop's use history, more-or-less represents a single instance of a
 * {@link SignShopSession}'s interaction with a shop.
 * <p>
 * This class is serialized in a <i>more compact</i> way than normal into a long array using
 * {@link #toArray()}
 *
...
public record HistoryEntry(long date, UUID customer, int amount, int earned, boolean wasBuy) {
  ...
  /**
   * Index of the most significant bits of customer's UUID in the {@link #toArray()} result.
   */
  private static final int INDEX_ID_MOST = 1;

  /**
   * Index of the least significant bits of customer's UUID in the {@link #toArray()} result,
   */
   ...
   * <p>
   * Just calls {@link #of(long[])} by casting the given tag to a {@link LongArrayTag}
   *
  ...
  /**
   * Loads the history entry from the given long array.
   *
   ...
   * <p>
   * Format: I'll briefly explain the format for this serialization
   * is: <pre>
  ...
  /**
   * Saves this entry into an NBT long array tag using the array returned by {@link #toArray()}
   *
