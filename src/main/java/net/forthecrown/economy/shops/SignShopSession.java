...
public class SignShopSession {
  ...
  /**
   * The amount of time, in ticks, a {@link SignShopSession} will stay alive until it `expires` and
   * is removed
   */
  ...
  /**
   * The customer interacting with the shop, in almost all cases, this will be a {@link User}
   * object.
   */
  @Getter
  ...
  /**
   * The amount of items exchanged during this session's life span
   */
  @Getter
  @Setter
  ...
  /**
   * The task that holds the mighty responsibility of killing this session by making it expired.
   * <p>
   ...
   * <p>
   * Calls {@link ShopInteraction#interact(SignShopSession)} for the session's type and passes
   * itself in as the parameter. After that it informs the user they've successfully interacted with
   * the shop.
   * <p>
   * It will also call {@link #growAmount(int)} with the shop's {@link #getExampleItem()} item
   * amount
   */
  public void run() {
  ...
  /**
   * Delays this session expiring by {@link #SESSION_TIME_OUT} ticks.
   * <p>
   * Will call the {@link #expire()} method once expiry timeout has ended
   */
  ...
  /**
   * Kills this session, removes it from the session map, logs this session, if {@link #shouldLog()}
   * returns true.
   * <p>
   * This will also send the customer a message about the session, if they interacted with the shop
   * for more than 5 times and, if the session's shop is a non-admin shop, it will tell the owner
   * that someone used their shop and tell them if the shop went out of stock
   */
  public void expire() {
  ...
  /**
   * Checks if this session should be logged by two checking the value of one of two variables,
   * {@link GeneralConfig#logAdminShop} or {@link GeneralConfig#logNormalShop}
   *
  ...
  /**
   * Gets the total amount of rhines earned from this session thus far
   *
