...
public class MultiplierMenu extends MenuPage {
  ...
  public MultiplierMenu(MenuPage parent) {
    ...
    initMenu(
        Menus.builder(Menus.sizeFromRows(5)).setTitle("EXP multipliers"),
        true
    );
  }
    ...
    builder.add(
        ...
        MenuNode.builder()
            .setItem((user, context) -> {
              ...
              boolean active = modifiers.isWeekend();
              ...
              item.setName(
                  "&eWeekend Multiplier " + GuildConfig.weekendModifier + "x"
              );

              item.addLore(
                  "&7Currently: " + (active ? "&aActive" : "&6Inactive")
              );

              item.addLore("")
                  .addLore("&7Activates for all players during the weekend");
