...
public class ExpModifiers {
  ...
  public void addMultiplier(UUID donator,
                            float modifier,
                            Duration duration,
                            MultiplierType type
  ) {
  ...
  public void activate(DonatorMultiplier multiplier) {
    ...
    multiplier.forEachAffected(user -> {
      ...
      if (active.size() == 1) {
        user.sendMessage(Messages.multiplierNowActive(mod));
        return;
      }

      // More than 1 active
      if (active.size() > 1) {
        user.sendMessage(Messages.multiplierIncremented(mod));
      }
    });
  }
  ...
  public void deactivate(DonatorMultiplier multiplier) {
    ...
    multiplier.forEachAffected(user -> {
      float mod = getModifier(user.getUniqueId());
      user.sendMessage(Messages.multiplierDecremented(mod));
    });
  }
