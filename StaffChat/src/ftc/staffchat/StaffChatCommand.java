...
public class StaffChatCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(args.length < 1) 
        {
        	sender.sendMessage("Usage: /sc <Message>");
        	return false;
        }
        ...
        for(Player p : Bukkit.getServer().getOnlinePlayers()){ //loops through all players and sends the one with staffchat perms the message
            if(p.hasPermission("ftc.staffchat")){
                p.sendMessage(ChatColor.GRAY + "[Staff] " + sender.getName() + ChatColor.GRAY + ChatColor.BOLD + " >" + ChatColor.RESET + ChatColor.translateAlternateColorCodes('&', message));
            }
        }
        return false;
    }
