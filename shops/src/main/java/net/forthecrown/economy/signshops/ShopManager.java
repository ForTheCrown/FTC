...
public class ShopManager {
   ...
   * <p>
   * This method will also check if the player should be shown the shop's inventory instead of
   * interacting with the shop as one normally would.
   * <p>
   * The interaction itself is logged most of the time and is conducted through
   * {@link SignShopSession} instances. The session itself doesn't run any shop logic or ensure the
   * user can actually use the shop, that job is left to the shop's {@link ShopInteraction}, which
   * is determined by the shop's type.
   *
  ...
  /**
   * Creates and a session for the given customer and shop. The created session is also added to the
   * session map
   *
  ...
  private SignShopSession createSession(User customer, SignShop shop) {
    ...
    var session = new SignShopSession(this, shop, customer);
    addSession(session);
  ...
  private void addShop(SignShop shop) {
  ...
  /**
   * Removes a shop from the loaded shops list
   *
   * @param shop The shop to remove
   */
  public void removeShop(SignShop shop) {
    var removed = loadedShops.remove(shop.getPosition());
