...
final class Interactions {
  ...
  public static final ShopInteraction ADMIN_BUY = new ShopInteraction() {
    ...
    public void interact(@NotNull SignShopSession session) {
      ...
      //Give item
      boolean mayEdit = SignShops.mayEdit(session.getShop(), session.getCustomer().getUniqueId());
      if (session.getShop().isResellDisabled() && !mayEdit) {
        SignShops.makeNonResellable(exampleItem);
      }
