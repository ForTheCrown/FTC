...
public class parrot extends CrownCommand {
    ...
    public boolean run(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0)
        {
            if (Pirates.plugin.parrots.containsValue(player.getUniqueId()))
            {
        ...
        if (player.getShoulderEntityLeft() != null && (!Pirates.plugin.parrots.containsValue(player.getUniqueId())))
        {
    ...
    private void removeOldParrot(Player player, Parrot parrot) {
        ...
        if (parrot != null)
        {
            Pirates.plugin.parrots.remove(parrot.getUniqueId());
            parrot.remove();
        }
    }
    ...
    private void makeParrot(Parrot.Variant color, Player player, Boolean silent) {
        ...
        Pirates.plugin.parrots.put(parrot.getUniqueId(), player.getUniqueId());
