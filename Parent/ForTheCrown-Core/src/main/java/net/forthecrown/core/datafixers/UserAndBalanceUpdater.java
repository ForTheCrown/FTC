...
public class UserAndBalanceUpdater implements DataFixer<UUID> {
    ...
    public UserAndBalanceUpdater(FtcCore core) throws IOException {
        this.userDir = new File(core.getDataFolder().getPath() + File.separator + "playerdata");
        this.logger = core.getLogger();
        balances = FtcCore.getBalances().getBalanceMap();
    ...
    public DataFixer<UUID> begin() {
        ...
        for (OfflinePlayer player: Bukkit.getOfflinePlayers()){
            boolean inBalances = balances.containsKey(player.getUniqueId());
    ...
    public void complete() {
        ...
        FtcCore.getBalances().setBalanceMap(balances);
