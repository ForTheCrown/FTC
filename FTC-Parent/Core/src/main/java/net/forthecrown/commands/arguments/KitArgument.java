...
public class KitArgument implements ArgumentType<Key> {
    ...
    public static Kit getKit(CommandContext<CommandSource> c, String argument){
        return ForTheCrown.getKitManager().get(c.getArgument(argument, Key.class));
    }
    ...
    public Key parse(StringReader reader) throws CommandSyntaxException {
        ...
        if(!ForTheCrown.getKitManager().contains(key)) throw UNKNOWN_KIT.createWithContext(GrenadierUtils.correctReader(reader, cursor), key);
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder, boolean ignoreChecks){
        if(ignoreChecks) return FtcSuggestionProvider.suggestRegistry(builder, ForTheCrown.getKitManager());

        try {
            return ForTheCrown.getKitManager().getSuggestions((CommandContext<CommandSource>) context, builder);
