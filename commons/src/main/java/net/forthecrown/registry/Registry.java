 ...
 * integer ID can be used then to index the value outside this registry.
 * <p>
...
public class Registry<V> implements Iterable<V> {
   ...
   */
  private final Object2ObjectMap<String, Holder<V>> byKey = new Object2ObjectOpenHashMap<>();

  /**
   * Value to entry lookup map
   */
  private final Object2ObjectMap<V, Holder<V>> byValue = new Object2ObjectOpenHashMap<>();
      ...
      throws IllegalArgumentException, NullPointerException
  {
    ...
    if (contains(holder.getKey())) {
      throw newException("Registry already contains mapping for '%s'",
          holder.getKey()
      );
    }
    ...
    if (holder.getId() < 0) {
      throw newException("Invalid index for entry '%s': %s",
   ...
   * @throws IllegalArgumentException If the registry is frozen
   */
  private boolean removeHolder(Optional<Holder<V>> optional) throws IllegalArgumentException {
    testFrozen();
  ...
  public void freeze() throws IllegalArgumentException {
    if (!isFreezingAllowed()) {
      throw newException("This registry does not allow freezing");
    }
   ...
   */
  public void setListener(RegistryListener<V> listener) throws IllegalArgumentException {
    Objects.requireNonNull(listener);

    if (this.listener != null) {
      throw newException("Registry listener already set!");
    }
  ...
  public @NotNull V orThrow(int id) throws IllegalArgumentException {
    return get(id).orElseThrow(() -> newException("Could not find value for ID %s", id));
  }
   ...
   * <p>
   * This method also uses a {@link IntSet} to ensure no duplicate entry IDs are selected while
   * looking for a valid entry
   *
  ...
  public @NotNull ImmutableList<V> values() {
    ...
    return ImmutableList.copyOf(Iterators.transform(it, Holder::getValue));
  }
  ...
  public static IllegalArgumentException unknownKey(Object key) {
    return newException("Couldn't find value for key: '%s'", key);
  }
  ...
  public static @NotNull String removeNamespace(@NotNull String s) throws IllegalArgumentException {
    ...
    if (after.isBlank()) {
      throw newException("Invalid key: '%s', blank after namespace", s);
    }

    return after;
  }

  private static IllegalArgumentException newException(String msg, Object... args) {
