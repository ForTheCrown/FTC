...
/**
 * A sister class to {@link ChunkedMap} to provide a chunk based spatial lookup that takes values
 * being located in different worlds into consideration, in essence, separating all given values
 * between the worlds they're in.
 *
 * @param <T> The map's type
 * @see ChunkedMap
 */
public class WorldChunkMap<T> implements CollisionLookup<T> {
  ...
  public void getColliding(World world, Bounds3i bounds3i, CollisionSet<T> out) {
    ...
    worldMap.getColliding(world, bounds3i, out);
  }
