...
public class MonthDayPeriod {
   ...
   * <p>
   * If {@link #isExact()} returns true, then this determines the date of the holiday, and
   * {@link #end} will be null.
   */
   ...
   * <p>
   * This maybe null, that means that this is an exact date period, meaning it only lasts a single
   * day. And in that case, {@link #start} will not be null.
   * <p>
   * {@link #isExact()} can be used to easily check if this value is null, aka, if this is an exact
   * period.
   */
   ...
   * @throws DateTimeParseException If either the start or end couldn't be parsed.
   ...
   * <p>
   * This method accepts a specific format for parsing, that being 'DD.MM' examples of accepted
   * input are:
   * <pre>
   * - 01.12
   * - 31.12
  ...
  /**
   * Creates a holiday period that's only active on the given month-day
   *
  ...
  /**
   * Creates a period between the given month days
   *
  ...
  public BinaryTag save() {
    if (isExact()) {
      return BinaryTags.stringTag(PARSER.format(start));
    }
  ...
  public static MonthDayPeriod load(JsonElement element) {
    ...
    if (array.size() > 2 || array.isEmpty()) {
      throw new IllegalArgumentException(
          String.format("Expected size [1..2], found: %s", array.size())
      );
    }
