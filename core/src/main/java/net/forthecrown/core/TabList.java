...
public final class TabList {
   ...
   * </pre>
   * <p>
   * This method is set up in this way to allow for the most optimized way of updating the tab menu
   * for each player. Currently, to update the tab menu for all players, this method has to only be
   * called 1, instead of being called for each user separately.
   *
  ...
  public static Triple<Component, Component, Component> createHeaderFooter() {
    ...
    for (var u : Users.getOnline()) {
      ...
      if (displayObj != null) {
        int score = displayObj.getScore(u.getName()).getScore();
