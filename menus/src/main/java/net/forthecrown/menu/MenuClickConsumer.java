...
public interface MenuClickConsumer {
  ...
  /**
   * Called when a user clicks in an inventory, or when the user clicks on a specific node,
   * determined by which class is implementing this interface.
   * <p>
   * If this class is being used by {@link MenuNode} then this represents that node's click
   * callback, otherwise it will be called for every click in a menu.
   *
   * @param user    The user clicking
   * @param context The inventory context
   * @param click   The click context, essentially a wrapper for the click event with some extended
   *                features
  ...
  interface Contextless extends MenuClickConsumer {
     ...
     * @throws CommandSyntaxException If something went wrong, the exception's failure message will
