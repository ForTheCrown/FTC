...
public final class AttackPhases {
  ...
  public static final AttackPhase ZOMBIES = new SummonPhase(
      (pos, world, context, boss) -> {
        Class<? extends Monster> spawnClass =
            boss.getRandom().nextInt(100) < EvokerConfig.zombies_skeletonChance
                ? Skeleton.class
                : Zombie.class;
  ...
  public static final AttackPhase ILLAGERS = new SummonPhase(
      (pos, world, context, boss) -> {
        double chance = EvokerConfig.illager_ravagerChance + Math.ceil(context.modifier());
        boolean ravager = boss.getRandom().nextInt(100) < chance;

        Class<? extends Raider> clazz = ravager
            ? Ravager.class
            : (boss.getRandom().nextBoolean() ? Pillager.class : Vindicator.class);
  ...
  public static void modifyHealth(
