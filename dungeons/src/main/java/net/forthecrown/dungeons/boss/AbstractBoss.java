...
public abstract class AbstractBoss implements DungeonBoss {
  ...
  public AbstractBoss(String name, Location spawn, WorldBounds3i room, SpawnTest requirement) {
    ...
    this.components =
        components.isEmpty() ? ObjectSets.emptySet() : ObjectSets.unmodifiable(components);
  }
  ...
  protected void createBossBar(BossContext context) {
    bossBar = Bukkit.createBossBar(getName(), BarColor.RED, BarStyle.SOLID, BarFlag.CREATE_FOG,
  ...
  protected void logSpawn(BossContext context) {
    ...
    LOGGER.info("{} spawned, context: players: [{}], difficulty: {}", getName(), players,
        context.modifier());
  }
}