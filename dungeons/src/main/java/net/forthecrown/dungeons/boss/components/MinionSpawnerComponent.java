...
public class MinionSpawnerComponent implements BossComponent<DungeonBoss> {
  ...
  public void onTick(DungeonBoss boss, BossContext context) {
    ...
    if (tick > spawnTickInterval) {
      ...
      // given spawner
      Vector3d random = spawns[boss.getRandom().nextInt(spawns.length)];

      Entity e = spawner.create(random, boss.getWorld(), context, boss);
  ...
  public interface MinionSpawner {
     ...
     */
    @NotNull Entity create(Vector3d pos, World world, BossContext context, DungeonBoss boss);
  }
}