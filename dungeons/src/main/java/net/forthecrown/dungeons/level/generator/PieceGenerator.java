...
public class PieceGenerator {
  ...
  public PieceGenerator(
    ...
    type.fillPotentials(this).filter(pair -> {
      ...
      if (flags.contains(RoomFlag.ROOT) || flags.contains(RoomFlag.BOSS_ROOM)) {
        return false;
      }

      // Ensure connectors have matching opening sizes
      GateData.Opening opening = getOrigin().getTargetGate().opening();
      ...
      return !data.successful.contains(pair.second()) && matchingOpenings > 0;
    })
  ...
  public StepResult generate() {
    ...
    if (sectionDepth > data.getOptimalDepth() && config.getRandom().nextInt(4) == 0) {
      return sectionDepthFailure();
    }
      ...
      while (it.hasNext()) {
        ...
        while (gIt.hasNext()) {
          ...
          if (exits.size() > type.getMaxExits(config)) {
            pickUniqueEntries(exits, gen.getConfig().getRandom(), type.getMaxExits(config))
          ...
          if (this.type == SectionType.ROOM) {
            ...
            if ((depth <= config.getMinDepth()
              ...
              pickUniqueEntries(exits, rand, keepOpen)
  ...
  public static <T> Set<T> pickUniqueEntries(List<T> list, Random random, int amount) {
    Preconditions.checkArgument(amount <= list.size());
