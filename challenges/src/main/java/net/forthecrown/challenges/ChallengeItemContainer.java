...
public class ChallengeItemContainer {
  ...
  public CompletableFuture<ItemStack> next(Random random) {
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      if (potentials.isEmpty()) {
        ...
        return findRandom(potentials.get(0), random, loopCounter);
      }
      ...
      while (ItemStacks.isEmpty(result) || used.contains(result)) {
        ...
        if (loopCounter[0] > MAX_SEARCH_ATTEMPTS) {
          LOGGER.warn(
   ...
   * that container
   */
  private ItemStack findRandom(ItemStack item, Random random, int[] loopCounter) {
