...
public final class AttackPhases {
    ...
    public static final AttackPhase ZOMBIES = new SummonPhase(
            (pos, world, context) -> {
                Class<? extends Monster> spawnClass = FtcUtils.RANDOM.nextInt(100) < EvokerVars.SKELETON_SPAWN_CHANCE.get() ?
                ...
                return world.spawn(new Location(world, pos.x, pos.y, pos.z), spawnClass, zombie -> {
                   ...
                   DungeonUtils.clearModifiers(speed);
    ...
    public static final AttackPhase ILLAGERS = new SummonPhase(
            (pos, world, context) -> {
                boolean ravager = FtcUtils.RANDOM.nextInt(100) < EvokerVars.RAVAGER_SPAWN_CHANCE.get() + Math.ceil(context.modifier());
                Class<? extends Raider> clazz = ravager ? Ravager.class : (FtcUtils.RANDOM.nextBoolean() ? Pillager.class : Vindicator.class);
                ...
                return world.spawn(
                        ...
                        raider -> {
                            ...
                            if (ItemStacks.isEmpty(mainHand)) return;
    ...
    static Team getMobTeam() {
        ...
        Team t = scoreboard.getTeam(EvokerVars.MOB_TEAM.get());

        if(t == null) {
            t = scoreboard.registerNewTeam(EvokerVars.MOB_TEAM.get());
