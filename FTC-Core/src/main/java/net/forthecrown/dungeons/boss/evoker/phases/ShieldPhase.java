...
public class ShieldPhase implements AttackPhase {
    ...
    public void onStart(EvokerBoss boss, BossContext context) {
        ...
        bar.setTitle("Shield down, attack now!");
        bar.setColor(BarColor.YELLOW);
        bar.setVisible(true);

        EvokerEffects.shieldLoseEffect(
                boss.getWorld(),
                MathUtil.toWorldEdit(boss.getBossEntity().getLocation()),
                boss.getRoom()
        );
    ...
    public void onEnd(EvokerBoss boss, BossContext context) {
        ...
        if(boss.getState() != EvokerState.DYING) {
            ...
            EvokerEffects.shieldGainEffect(boss.getWorld(), MathUtil.toWorldEdit(entity.getLocation()), boss.getRoom());
        }
    ...
    public void onTick(EvokerBoss boss, BossContext context) {
        int maxTicks = EvokerVars.SHIELD_PHASE_LENGTH.get();
        tick++;
