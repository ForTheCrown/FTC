...
public class EvokerBoss extends KeyedBossImpl implements SingleEntityBoss {
    ...
    void onSummoningFinish() {
        ...
        evoker.getPersistentDataContainer().set(Bosses.BOSS_TAG, PersistentDataType.STRING, key().asString());
        evoker.setLootTable(LootTables.EMPTY.getLootTable());

        Location l = evoker.getLocation();
        EvokerEffects.impactEffect(getWorld(), Vector3.at(l.getX(), l.getY(), l.getZ()));
    ...
    public void kill(boolean force) {
        ...
        if(!force) {
            Location l = evoker.getLocation();
            Vector3 pos = Vector3.at(l.getX(), l.getY(), l.getZ());
            EvokerEffects.shockwave(getWorld(), pos);
            EvokerRewards.create(this, currentContext);
    ...
    protected void tick() {
        ...
        if(transitionTick != NO_TRANSITION && !animationOngoing()) {
            transitionTick++;

            if(transitionTick >= EvokerVars.PHASE_TRANSITION.get()) {
                nextPhase(false);
            }
        }
    ...
    void createPhaseOrder() {
        ...
        Collections.shuffle(phases, FtcUtils.RANDOM);
    ...
    private AttackPhase getRand() {
        return AttackPhases.SELECTABLE_PHASES[FtcUtils.RANDOM.nextInt(AttackPhases.SELECTABLE_PHASES.length)];
    }

    private Evoker createEvoker(BossContext context) {
        return getWorld().spawn(getSpawn(), Evoker.class, evoker -> {
            AttackPhases.modifyHealth(evoker, EvokerVars.BASE_HEALTH.get(), context);
    ...
    public void broadcast(boolean allowCancellation, BossMessage... msgs) {
        ...
        if(FtcUtils.RANDOM.nextBoolean() && allowCancellation) return;
        ...
        while (iterator.hasNext()) {
            ...
            if(iterator.hasNext())  builder.append(Component.newline());
        }
    ...
    protected void giveRewards(Player player) {
        DungeonUtils.giveOrDropItem(player.getInventory(), player.getLocation(), BossItems.EVOKER.item());
    }
    ...
    public void setAttackingAllowed(boolean attackingAllowed) {
        this.attackingAllowed = attackingAllowed;
    }
    ...
    public static Team getBossTeam() {
        ...
        Team t = scoreboard.getTeam(EvokerVars.BOSS_TEAM.get());

        if(t == null) {
            t = scoreboard.registerNewTeam(EvokerVars.BOSS_TEAM.get());
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        if (!event.getEntity().equals(getBossEntity())) return;
        ...
        if(newHealth < EvokerVars.DEATH_PHASE_HEALTH.get()) {
            event.setCancelled(true);
