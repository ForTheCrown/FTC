...
public class NormalAttackPhase implements AttackPhase {
    ...
    public void onTick(EvokerBoss boss, BossContext context) {
        ...
        if(boss.getBossEntity().getSpell() == Spellcaster.Spell.NONE) {
            boss.getBossEntity().setSpell(FtcUtils.RANDOM.nextBoolean() ? Spellcaster.Spell.FANGS : Spellcaster.Spell.SUMMON_VEX);
        }

        if(tick >= EvokerVars.NORMAL_PHASE_LENGTH.get()) {
            boss.nextPhase(true);
        } else {
            double progress = (double) tick / (double) EvokerVars.NORMAL_PHASE_LENGTH.get();
