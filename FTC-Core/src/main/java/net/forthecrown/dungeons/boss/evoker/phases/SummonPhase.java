...
public class SummonPhase implements AttackPhase {
    ...
    public void onTick(EvokerBoss boss, BossContext context) {
        ...
        spawnTick++;
        if (spawnTick <= EvokerVars.TICKS_BETWEEN_SPAWNS.get()) return;

        spawnTick = 0;

        double[] rawPos = SPAWNS[currentIndex++];
        double randomRange = rawPos[3];
        double xOffset = randomRange == 0 ? 0 : FtcUtils.RANDOM.nextDouble(-randomRange + 0.1D, randomRange);
        double zOffset = randomRange == 0 ? 0 : FtcUtils.RANDOM.nextDouble(-randomRange + 0.1D, randomRange);
        ...
        EvokerEffects.summoningEffect(boss.getWorld(), MathUtil.toWorldEdit(e.getLocation()), e.getHeight(), e.getWidth());
    }
    ...
    public static void shuffleSpawns() {
        ...
        for (int i = 0; i < length; i++) {
            int newIndex = FtcUtils.RANDOM.nextInt(length);
