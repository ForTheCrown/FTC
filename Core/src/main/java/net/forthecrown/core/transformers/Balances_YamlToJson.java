...
public class Balances_YamlToJson {
    public static void checkAndRun() {
        if(!new File(Crown.dataFolder().getPath() + File.separator + "balance.yml").exists()) return;
    ...
    private Balances_YamlToJson() throws IOException {
        this.yamlFile = new File(Crown.dataFolder().getPath() + File.separator + "balance.yml");
        this.jsonFile = new File(Crown.dataFolder().getPath() + File.separator + "balances.json");
