...
public class HelpHelp extends FtcCommand {
                ...
                .then(literal("specific")
                        .then(argument("command", StringArgumentType.word())
                                .executes(c -> {
                                    ...
                                    if(FtcCommands.BY_NAME.containsKey(cmd)){
                                        FtcCommand lookup = FtcCommands.BY_NAME.get(cmd);
    ...
    private List<Component> availableCommands(CommandSource sender){
        Collection<FtcCommand> commands = FtcCommands.BY_NAME.values();
