...
public class TreasureShulker extends AbstractJsonSerializer {
    ...
    public TreasureShulker(){
        ...
        reload();
        Crown.logger().info("Treasure Shulker loaded");

        Crown.getDayUpdate().addListener(() -> {
            alreadyFound.clear();
            relocate();
        });
    }
    ...
    public void kill(){
        if(currentID == null && location == null){
            Crown.logger().warning("Tried to kill treasure shulker, but both location and id were null");
            return;
        }
    ...
    public void moveLocToRandom(){
        ...
        while (loc.getBlock().getType() == Material.WATER) {
            ...
            safeGuard--;
            if(safeGuard < 0) {
                Crown.logger().warning("Couldn't find valid location for TreasureShulker in 300 attempts");
                break;
            }
        }
        ...
        public boolean giveRewards(Player player){
            ...
            player.sendMessage(
                    Component.translatable("pirates.shulker.found",
                            NamedTextColor.GRAY,
                            display(),
                            FtcFormatter.piratePoints(rarity.ppReward)
                    )
            );
            return true;
        }
