...
public class ResourceWorldTracker {
  ...
  ResourceWorldTracker(RwPlugin plugin) {
    this.plugin = plugin;
    this.directory = PathUtil.pluginPath(plugin, "rw_data");
  ...
  public void reset() {
    ...
    PathUtil.safeDelete(directory, true, true)
  ...
  private void pushbackTask(McRegionPos pos, WorldRegion region) {
    ...
    region.unloadTask = Tasks.runLaterAsync(() -> unloadSection(pos), config.sectionRetentionTime);
  }
